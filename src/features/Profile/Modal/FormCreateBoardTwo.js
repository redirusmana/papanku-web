...
import { apiCreateBoard, apiUpdateBoard } from "../action"; //apiAddFriend,
...
class FormCreateBoard extends React.PureComponent {
  ...
  handleSubmit = async (values, actions) => {
    const { history, idBoard } = this.props;
    try {
      this._requestSource = api.generateCancelToken();

      const response = idBoard
        ? await apiUpdateBoard(values, idBoard, this._requestSource.token)
        : await apiCreateBoard(values, this._requestSource.token);
